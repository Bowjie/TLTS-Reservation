 ...
 <tr>
  <?php 
	...
	foreach($explodedate as $date){
			$exdate=$date;
		// $date
		$rsql1 = "SELECT COUNT(equipment_id) AS countreserved FROM reservation WHERE equipment_id='".$equipment_id."' AND dates = '$exdate' AND status ='approved' OR status = 'borrowed' OR status = 'damaged' OR status= 'missing' ";
	...
	foreach($equipmentResult as $getrow){
		// $date
		// print_r("asdasd");
		$rsql1 = "SELECT COUNT(equipment_id) AS countreserved FROM reservation WHERE equipment_id='".$getrow[0]."' AND dates IN($exdate) AND status ='approved' OR status = 'borrowed' OR status = 'damaged' OR status= 'missing' ";  
		$eqres1 = $mysqli->query($rsql1);
		$eqres2= $eqres1->fetch_assoc();
		$totalres = $eqres2['countreserved'];
...
Instructional Materials: <select id=e1 name="ins[]" multiple style="width: 150px;">
		...
		while($row=$result->fetch_assoc())
                          {  
    echo "<option value=".$row['cdid'].">".$row['eqtype']."</option>";
    }
    ?>
   ...
   <div class="container-fluid">
	 <ol class="breadcrumb">
      ...
      <div class="container">
      	<h3>Warning: Reservation is only 7am to 8pm</h3><br>
